...
const userRegisterSchema = zod_1.z.object({
    name: zod_1.z.string().min(3, "Nama harus lebih dari 3 karakter."),
...
const productSchema = zod_1.z.object({
    name: zod_1.z.string().min(3, { message: "Nama produk harus memiliki setidaknya 3 karakter" }),
    price: zod_1.z.number().positive({ message: "Harga harus bernilai positif" }),
    stock: zod_1.z.number().int().nonnegative({ message: "Stok tidak boleh negatif" }),
    categoryId: zod_1.z.number().int().positive({ message: "Kategori ID tidak valid" }),
    imageUrl: zod_1.z.string().min(1, { message: "URL gambar diperlukan" }),
    description: zod_1.z.string().optional(),
    features: zod_1.z.array(zod_1.z.string()).optional(),
    specifications: zod_1.z.record(zod_1.z.any()).optional(),
});
const updateProductSchema = zod_1.z.object({
    name: zod_1.z.string().min(3, { message: "Nama produk harus memiliki setidaknya 3 karakter" }).optional(),
    price: zod_1.z.number().positive({ message: "Harga harus bernilai positif" }).optional(),
    stock: zod_1.z.number().int().nonnegative({ message: "Stok tidak boleh negatif" }).optional(),
    categoryId: zod_1.z.number().int().positive({ message: "Kategori ID tidak valid" }).optional(),
    imageUrl: zod_1.z.string().min(1, { message: "URL gambar diperlukan" }).optional(),
    description: zod_1.z.string().optional(),
    features: zod_1.z.array(zod_1.z.string()).optional(),
    specifications: zod_1.z.record(zod_1.z.any()).optional(),
});
const newsSchema = zod_1.z.object({
    title: zod_1.z.string().min(5, { message: "Judul berita harus memiliki setidaknya 5 karakter" }),
    excerpt: zod_1.z.string().min(10, { message: "Kutipan berita harus memiliki setidaknya 10 karakter" }),
    content: zod_1.z.string().min(20, { message: "Konten berita harus memiliki setidaknya 20 karakter" }),
    imageUrl: zod_1.z.string().min(1, { message: "URL gambar berita diperlukan" }),
    author: zod_1.z.string().min(3, { message: "Nama penulis harus memiliki setidaknya 3 karakter" }),
});
const updateNewsSchema = zod_1.z.object({
    title: zod_1.z.string().min(5, { message: "Judul berita harus memiliki setidaknya 5 karakter" }).optional(),
    excerpt: zod_1.z.string().min(10, { message: "Kutipan berita harus memiliki setidaknya 10 karakter" }).optional(),
    content: zod_1.z.string().min(20, { message: "Konten berita harus memiliki setidaknya 20 karakter" }).optional(),
    imageUrl: zod_1.z.string().min(1, { message: "URL gambar berita diperlukan" }).optional(),
    author: zod_1.z.string().min(3, { message: "Nama penulis harus memiliki setidaknya 3 karakter" }).optional(),
    slug: zod_1.z.string().optional(),
});
const profileSchema = zod_1.z.object({
    name: zod_1.z.string().min(3, "Nama harus memiliki setidaknya 3 karakter.").max(50),
    phoneNumber: zod_1.z.string().max(15).optional().nullable(),
    address: zod_1.z.string().max(255).optional().nullable(),
});
const reviewSchema = zod_1.z.object({
    productId: zod_1.z.number(),
    orderItemId: zod_1.z.number(),
    rating: zod_1.z.number().min(1, "Rating harus diisi").max(5),
    comment: zod_1.z.string().min(10, "Komentar minimal harus 10 karakter."),
});
// Helper for verification token creation
async function createAndSendToken(email) {
    ...
    await (0, mail_1.sendVerificationEmail)(email, token);
}
async function createAndSendPasswordResetToken(email) {
    const token = crypto_1.default.randomBytes(3).toString('hex').toUpperCase();
    const expires = new Date(new Date().getTime() + 3600 * 1000); // 1 hour
    await prisma_1.default.verificationToken.deleteMany({ where: { email } });
    await prisma_1.default.verificationToken.create({
...
app.post('/api/auth/register', async (req, res) => {
    try {
        ...
        const hashedPassword = await (0, bcrypt_1.hash)(validatedData.password, 10);
...
app.post('/api/auth/verify-email', async (req, res) => {
    try {
        ...
        const verificationToken = await prisma_1.default.verificationToken.findUnique({ where: { token } });
        ...
        const user = await prisma_1.default.user.findUnique({ where: { email: verificationToken.email } });
        if (!user)
        ...
        await prisma_1.default.verificationToken.delete({ where: { id: verificationToken.id } });
...
app.get('/api/products', async (req, res) => {
    try {
        ...
        const limitVal = parseInt(req.query.limit || '9', 10);
        ...
        if (req.query.categoryId) {
            if (Array.isArray(req.query.categoryId)) {
                categoryIds = req.query.categoryId.map((id) => parseInt(id, 10)).filter((id) => !isNaN(id));
            }
            else {
                categoryIds = [parseInt(req.query.categoryId, 10)].filter((id) => !isNaN(id));
            }
        }
        ...
        const totalProducts = await prisma_1.default.product.count({ where });
...
app.post('/api/products', auth_1.authenticateJWT, (0, auth_1.requireRole)(['ADMIN']), async (req, res) => {
    try {
        ...
        const newProduct = await prisma_1.default.product.create({
            data: {
                ...
                description: validatedData.description ? (0, sanitizer_1.sanitizeInput)(validatedData.description) : undefined,
...
app.put('/api/products/:id', auth_1.authenticateJWT, (0, auth_1.requireRole)(['ADMIN']), async (req, res) => {
    try {
        const id = parseInt(req.params.id, 10);
        if (isNaN(id))
            return res.status(400).json({ message: 'Invalid product ID' });
...
app.delete('/api/products/:id', auth_1.authenticateJWT, (0, auth_1.requireRole)(['ADMIN']), async (req, res) => {
    try {
        const id = parseInt(req.params.id, 10);
        if (isNaN(id))
            return res.status(400).json({ message: 'Invalid product ID' });
...
app.post('/api/categories', auth_1.authenticateJWT, (0, auth_1.requireRole)(['ADMIN']), async (req, res) => {
    try {
        const { name } = req.body;
        if (!name || typeof name !== 'string') {
            return res.status(400).json({ message: "Nama kategori diperlukan dan harus berupa text" });
        }
        const sanitizedName = (0, sanitizer_1.sanitizeInput)(name);
...
app.delete('/api/categories/:id', auth_1.authenticateJWT, (0, auth_1.requireRole)(['ADMIN']), async (req, res) => {
    try {
        const id = parseInt(req.params.id, 10);
        if (isNaN(id))
            return res.status(400).json({ message: 'Invalid category ID' });
        const productsInCategory = await prisma_1.default.product.count({ where: { categoryId: id } });
...
// GET /api/cart
app.get('/api/cart', auth_1.authenticateJWT, async (req, res) => {
    try {
        if (!req.user)
            return res.status(401).json({ message: 'Unauthorized' });
...
// POST /api/cart
app.post('/api/cart', auth_1.authenticateJWT, async (req, res) => {
    try {
        if (!req.user)
            return res.status(401).json({ message: 'Unauthorized' });
        ...
        const cartItem = await prisma_1.default.cartItem.upsert({
...
// PUT /api/cart
app.put('/api/cart', auth_1.authenticateJWT, async (req, res) => {
    try {
        if (!req.user)
            return res.status(401).json({ message: 'Unauthorized' });
        const { productId, quantity } = req.body;
        ...
        const cart = await prisma_1.default.cart.findUnique({ where: { userId: req.user.id } });
        if (!cart)
...
// POST /api/checkout
app.post('/api/checkout', auth_1.authenticateJWT, async (req, res) => {
    try {
        if (!req.user)
            return res.status(401).json({ message: 'Unauthorized' });
        ...
        const productsFromDb = await prisma_1.default.product.findMany({
            where: { id: { in: productIds } },
        });
        const productMap = new Map(productsFromDb.map((p) => [p.id, {
                id: p.id,
                name: p.name,
                price: Number(p.price),
                stock: p.stock,
        ...
        const snapToken = await (0, midtrans_1.createSnapTransaction)({
...
app.get('/api/orders/all', auth_1.authenticateJWT, (0, auth_1.requireRole)(['ADMIN']), async (req, res) => {
    try {
        const page = parseInt(req.query.page || '1');
        const limit = 10;
        const skip = (page - 1) * limit;
...
app.get('/api/orders/pending-count', auth_1.authenticateJWT, (0, auth_1.requireRole)(['ADMIN']), async (req, res) => {
    try {
        const pendingCount = await prisma_1.default.order.count({
...
// GET /api/orders/:id
app.get('/api/orders/:id', auth_1.authenticateJWT, async (req, res) => {
    try {
        if (!req.user)
            return res.status(401).json({ message: 'Unauthorized' });
...
app.put('/api/orders/:id', auth_1.authenticateJWT, (0, auth_1.requireRole)(['ADMIN']), async (req, res) => {
    try {
        const id = parseInt(req.params.id);
...
// ─── PROFILE ENDPOINTS ──────────────────────────────────────────────────────
// GET /api/profile
app.get('/api/profile', auth_1.authenticateJWT, async (req, res) => {
    try {
        if (!req.user)
            return res.status(401).json({ message: 'Unauthorized' });
        const user = await prisma_1.default.user.findUnique({
...
// PUT /api/profile
app.put('/api/profile', auth_1.authenticateJWT, async (req, res) => {
    try {
        if (!req.user)
            return res.status(401).json({ message: 'Unauthorized' });
...
app.get('/api/profile/orders', auth_1.authenticateJWT, async (req, res) => {
    try {
        if (!req.user)
            return res.status(401).json({ message: 'Unauthorized' });
...
app.get('/api/reviews/check', auth_1.authenticateJWT, async (req, res) => {
    try {
        if (!req.user)
            return res.status(401).json({ message: 'Unauthorized' });
        const productId = parseInt(req.query.productId, 10);
        const orderItemId = parseInt(req.query.orderItemId, 10);
...
// POST /api/reviews
app.post('/api/reviews', auth_1.authenticateJWT, async (req, res) => {
    try {
        if (!req.user)
            return res.status(401).json({ message: 'Unauthorized' });
        ...
        const newReview = await prisma_1.default.$transaction(async (tx) => {
...
app.get('/api/news/:id', async (req, res) => {
    try {
        ...
        const newsItem = await prisma_1.default.news.findUnique({ where: { id } });
        if (!newsItem)
            return res.status(404).json({ message: 'News article not found' });
        return res.json(newsItem);
    }
...
app.post('/api/news', auth_1.authenticateJWT, (0, auth_1.requireRole)(['ADMIN']), async (req, res) => {
    try {
        const validatedData = newsSchema.parse(req.body);
        const slug = validatedData.title.toLowerCase().replace(/\s+/g, '-').replace(/[^\w-]+/g, '');
        const newNews = await prisma_1.default.news.create({
            data: {
                title: validatedData.title,
                excerpt: validatedData.excerpt,
                content: validatedData.content ? (0, sanitizer_1.sanitizeInput)(validatedData.content) : '',
                imageUrl: validatedData.imageUrl,
...
app.put('/api/news/:id', auth_1.authenticateJWT, (0, auth_1.requireRole)(['ADMIN']), async (req, res) => {
    try {
        const id = parseInt(req.params.id, 10);
        if (isNaN(id))
            return res.status(400).json({ message: 'Invalid news ID' });
        ...
        const updatedNews = await prisma_1.default.news.update({
            ...
            data: {
                title: validatedData.title,
                excerpt: validatedData.excerpt,
                content: validatedData.content ? (0, sanitizer_1.sanitizeInput)(validatedData.content) : undefined,
                imageUrl: validatedData.imageUrl,
                author: validatedData.author,
...
app.delete('/api/news/:id', auth_1.authenticateJWT, (0, auth_1.requireRole)(['ADMIN']), async (req, res) => {
    try {
        const id = parseInt(req.params.id, 10);
        if (isNaN(id))
            return res.status(400).json({ message: 'Invalid news ID' });
...
app.get('/api/payment/snap-token/:orderId', auth_1.authenticateJWT, async (req, res) => {
    try {
        if (!req.user)
            return res.status(401).json({ message: 'Unauthorized' });
...
// POST /api/upload
app.post('/api/upload', auth_1.authenticateJWT, upload.single('file'), async (req, res) => {
    if (!req.file) {
...
app.get('/api/dashboard/stats', auth_1.authenticateJWT, (0, auth_1.requireRole)(['ADMIN']), async (req, res) => {
    try {
        const currentYear = new Date().getFullYear();
        const startOfYear = new Date(currentYear, 0, 1);
        const [totalRevenueAggr, totalSalesAggr, totalCustomersCount, totalProductsCount, topSalesGrouped, newCustomersThisYear, paidOrdersThisYear] = await Promise.all([
        ...
        const topProductsInfo = await prisma_1.default.product.findMany({
        ...
        const getNewCustomersData = (users) => {
